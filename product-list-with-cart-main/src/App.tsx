...
function App() {
  ...
  const [orderDone, setDone] = useState<boolean>(false);
  ...
  const total = function () {
    const price = cart.reduce((acc, data) => acc + data.price * data.count, 0);
    setTotalPrice(price);
  };
  ...
  const delProduct = function (product: cartProduct) {
    setCart(cart.filter((data) => data.name !== product.name));
  };
  ...
  return (
    <div className="bg-rose-50 w-auto relative">
      <main className=" max-lg:w-80 mx-auto px-3 py-8 flex max-lg:flex-col flex-row justify-center">
        <div className="lg:w-[600px]">
          <span className="font-bold text-4xl text-red-950">Desserts</span>
          <div className="grid max-lg:grid-cols-1 grid-cols-3 gap-6 w-full">
        ...
        <div
          id="cart"
          className="lg:w-[400px] bg-white p-8 rounded-xl mt-5 lg:ml-5 h-fit"
        >
          <p className="text-red-600 text-xl font-bold">
            Your Cart ({cart?.length})
          </p>
          {cart.length === 0 ? (
            <div id="emptyCart" className="flex flex-col items-center">
              <svg
            ...
            <div className="flex flex-col w-full">
              {cart.map((data, index) => {
                return (
                  <div key={index} className="border-b-2 py-3 relative">
                    <p className="font-semibold text-rose-950">{data.name}</p>
                    <div>
                      <span className="font-semibold text-red-600 mr-2">
                        {data.count}x{" "}
                      </span>
                      <span className="font-thin text-rose-950 mr-2">
                    ...
                    >
                      <div className="border rounded-full p-1 border-rose-950">
                        <svg
                          className="text-rose-950 font-semibold"
                          xmlns="http://www.w3.org/2000/svg"
                          width="10"
                          height="10"
                          fill="none"
              ...
              <div
                id="totalPrice"
                className="py-4 flex flex-row items-center justify-between text-rose-950"
              >
                Order Total
                <span className="right-0 font-bold text-2xl">
                  ${totalPrice.toFixed(2)}
                </span>
              </div>
              <span
                id="ad"
                className="bg-red-50 px-2 py-3 flex flex-row rounded-md mb-5"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="21"
              ...
              <button
                className="border border-solid border-rose-900 rounded-full py-3 w-full bg-red-700 text-white"
                onClick={() => {
                  setDone(true);
                }}
              >
      ...
      {orderDone ? (
        <div
          id="OrderFinish"
          className="absolute w-full h-full bg-black bottom-0 bg-opacity-50 flex justify-center"
        >
          <div
            className="fixed bottom-0 w-full h-4/5 bg-white rounded-t-2xl pt-14 px-5  text-rose-950 
          lg:w-[530px] lg:bottom-24 lg:pt-8 lg:rounded-b-2xl
          "
          >
            <svg
              width="48"
            ...
            <div className="my-5 font-bold text-5xl lg:mb-0">
              Order {desktop ? null : <br></br>}Confirmed
            </div>
            <span>We hope you enjoy your food!</span>
            <div className="h-4/6 mt-5  ">
              <div className="flex flex-col w-full h-full">
                <div className="bg-rose-50 h-full overflow-y-auto p-4">
                  {cart.map((data, index) => {
                    console.log(data);
                    return (
                      <div
                        key={index}
                        className="border-b-2 py-2 relative flex flex-row items-center justify-between"
                      >
                        <div className="flex flex-row">
                          <img src={data.image.thumbnail} className="mr-5" />
                ...
                <button
                  className="border border-solid border-rose-900 rounded-full py-3 w-full bg-red-700 text-white mt-6"
                  onClick={() => {
                    newOrder();
                  }}
                >
